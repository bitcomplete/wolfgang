...
- **Bus-side decomposition is externally validated.** D7 (bus decomposes claims; agents never author the graph that governs them) matches icat-agent's message-passing-not-shared-context result; decomposition and delegation constraints are precisely what agents can't be trusted to self-report. (Corrected 2026-07-29: arXiv:2606.31498 documents audit/replay, escalation, and deliberation gaps in MCP/A2A/ACP — the delegation-constraint gap rests on the orchestration survey, not that paper.) (`notes/research-arxiv-multiagent-sdlc.md` §1.3, §4).
...
**R6. Design plan-review as the junior's mentorship channel.** Evidence-rich plan cards, reasons required on high-risk approvals, one-line close-out notes feeding the KB. The literature says the junior will mis-calibrate in both directions (corrected 2026-07-29); the design must make blind approval harder than informed approval — and informed approval easier than freezing.
