...
6. **Juniors over-trust agents.** arXiv:2602.00496: juniors accept agent output readily, sometimes without comprehension; Anthropic's skill-formation study: passive AI use impairs debugging skill. The release's user is *predicted by the literature* to over-approve.
...
9. **Minor factual frictions:** the "Hive" competitor brief was a misnomer (it's Buzz); Greenwood's $22k/yr governance figure is modeled-not-measured (externally consistent but unconfirmed — label it); several headline stats (89% cascade prevention, SREGym ranges) were read from abstracts and need PDF verification before external use.
